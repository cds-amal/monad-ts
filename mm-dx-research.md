...
**Functional - Use in component:**
```typescript
function MyComponent() {
  const enabled = useFeatureFlag('enableFlaggedAddressExplanation')
  //                             ^ TypeScript autocomplete, typo = compile error
...
**Functional - Disable feature globally:**
```typescript
// Change ONE line in defaults:
export const defaultFlags: FeatureFlags = {
  enableFlaggedAddressExplanation: false,  // Feature disabled everywhere
}
```

---

**Imperative - No infrastructure, scattered checks:**
```typescript
// Component A
...
function Sidebar() {
  if (window.FEATURE_FLAGS?.enableFlaggedExplanation) {  // Global object?
    // show feature
  }
}
```

**Imperative - Disable feature:**
```bash
