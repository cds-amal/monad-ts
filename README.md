...
**The short version:** The functional approach needed 2.6x more code upfront, but it was the only one that could ship iOS support without starting over. It also made feature flags trivial to add, while the imperative approach would've required retrofitting infrastructure.
...
| Metric | Functional (mm-monad_03) | Imperative (mm-imperative_01) |
|--------|--------------------------|-------------------------------|
| Total new code | ~864 LOC | ~327 LOC |
| Reusable primitives | 7 (Dropdown, Validators, ThemeOps, groupBy, adapters, EnvironmentContext, FeatureFlagsContext) | 1 (AccountBadge) |
...
The functional approach is slower at first, but the crossover happens around day 15. After that, adding features to the functional codebase is faster than fighting the imperative one.
...
└── metamask-baseline                               (MDS foundation)
    ...
    └── mm-monad_01 → mm-monad_02 → mm-monad_03    (adapters over MDS)
```
...
The full analysis is in [mm-dx-research.md](./mm-dx-research.md) on the mm-monad branches and duplicated on master. It covers:
...
- The 155 LOC of infrastructure that enables 500+ LOC worth of capabilities
